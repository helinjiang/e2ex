...
class E2ex {
  ...
  constructor(args) {
    ...
    const base = path.join(osenv.home(), './.e2ex');
    const rcPath = path.join(base, '.e2exrc.yml');
    ...
    this.config = utils.parseYaml(rcPath);            // Read e2ex local config.
  ...
  init() {
    ...
    this.log.debug('E2ex version: %s', chalk.magenta(this.version));
  ...
  /**
   * Load a plugin with vm module and inject e2ex variable,
   * e2ex is an instance and has context environment.
   *
  ...
  loadPlugin(pluginPath, callback) {
    ...
    return fs.readFile(pluginPath).then((script) => {
      ...
      // Inject e2ex variable
      script = '(function(exports, require, module, __filename, __dirname, e2ex){' +
        script + '});';
