...
class Upgrade {
  ...
  checkCore() {
    ...
    return pkgJson('e2ex-cli', 'latest', registry).then(json => {
      ...
      if (!semver.satisfies(version, compatibleVersion)) {
        log.info(`您当前使用的e2ex版本 ${version}, e2ex要求使用的版本为 ${compatibleVersion}, 将会使用全量更新策略`);

        const loading = new Loading('正在全量更新cli core，请稍等');
        return self.execNpmCommand('install', 'e2ex-cli', true).then(function (result) {
          if (!result.code) {
            loading.success();
            log.info(`已经自动升级到e2ex的最新版本${ltsVersion}`);
          } else {
  ...
  getInstalledPlugins() {
    ...
    return fs.exists(packagePath).then(function (exist) {
      ...
      // Ignore plugins whose name is not started with "e2ex-plugin-"
      if (!/^e2ex-plugin-|^@[^/]+\/e2ex-plugin-|^generator-|^@[^/]+\/generator-/.test(name)) return false;
