    ...
    builderWebpack3.getWebpackConfig()
        .then((webpackConfig) => {
            ...
            builderWebpack3.runBuild(webpackConfig, () => {
                ...
                // 插入 jQuery
                prependCodePromiseList.push(getJqueryCode());

                // 获得所有的代码之后，追加在头部
                if (prependCodePromiseList.length) {
                    Promise.all(prependCodePromiseList)
                        .then((result) => {
                            prependCodeToDistFile(configParams.distPath, result.join(';'));
                        });
                }
            });
        })
...
function getJqueryCode() {
    return new Promise((resolve, reject) => {
        fs.readFile(path.join(__dirname, '../../../node_modules/jquery/dist/jquery.slim.min.js'), 'utf8', (err, data) => {
            if (err) {
                return reject(err);
            }
...
function prependCodeToDistFile(distPath, code) {
